...
export function AddTierButton({ initialTierCount = 0 }: AddTierButtonProps) {
  ...
  const handleAddTier = () => {
    ...
    newTier.innerHTML = `
      <div>
        <label for="tier-min-${newTierCount}" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Quantité Minimale</label>
      ...
      <div>
        <label for="tier-max-${newTierCount}" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Quantité Maximale</label>
        <input id="tier-max-${newTierCount}" name="tier-max-${newTierCount}" type="number" min="1" placeholder="Laisser vide pour illimité" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" />
      </div>
      <div>
        <label for="tier-price-${newTierCount}" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Prix par Unité (MAD)</label>
